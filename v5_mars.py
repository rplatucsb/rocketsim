...
def main(thrust_lbf,innerRadius_in,fuel_mass_lb,dry_rocket_mass_lb):
    ...
    def cd_altitude_calc():
        ...
        drag_v5_list = [i*1.00 for i in drag_v5_list_m]
    ...
    while current_impulse <= total_impulse:
        ...
        current_cg = cg_v_t()
        thrust = 1000 * thrust_change()

        if height <= rail_height:
            acceleration_y = ( thrust - force_gravity - force_drag ) / total_rocket_mass #finds resultant acceleration
        else:
            theta, omega, alpha = rotation(wind_speed)
            acceleration_y = ( thrust * cos(theta) - force_gravity - force_drag * cos(theta) ) / total_rocket_mass #finds resultant acceleration
