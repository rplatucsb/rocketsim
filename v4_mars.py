...
from math import pi,pow,exp,sqrt,sin,cos,atan
...
def main(thrust_lbf,innerRadius_in,fuel_mass_lb,dry_rocket_mass_lb):
    ...
    def cg_v_t(dry_rocket_mass_lb, dry_rocket_cg, fuel_mass, tank_bottom_position, tank_height, burn_time, t):
        ...
        if t >= burn_time:
            ...
            current_fuel_mass = fuel_mass - (t/burn_t)*fuel_mass
            fuel_mass_cg = tank_bottom_pos + (tank_height*(1 - (t/burn_time)))/2
            return (current_fuel_mass*fuel_mass_cg + dry_rocket_mass*dry_rocket_cg)\
                       /(current_fuel_mass+dry_rocket_mass)

        # all of this code needs to be updated with actual values and then confirmed
    ...
    def center_of_pressure():
        ...
        body_a = 6.5*(115.1) #projected body area
        body_d = 32.5+(115.1/2) #body centroid distance
        fint_a = ((6.73*6.73)/2)*2 #first fin triangle area
        fint_d = 32.5+115.1-24+6.73/2 #centroid first fin triangle
        finr_a = (6.73*(16.09))*2 #rectangular fin portion area
        finr_d = 32.5+115.1-24+6.73+16.09/2 #centroid rectangular fin area
        finbt_a = 1.18*6.73*0.5*2 #last small triangle on fin area
        finbt_d = 32.5+115.1-(1.18*2)/3 #last small traingle centroid distance

        center_of_pressure = (nose_a[0]*nose_d+body_a*body_d+fint_a*fint_d+finr_a*finr_d+finbt_a*finbt_d)/(nose_a[0]+body_a+fint_a+finr_a+finbt_a)
    ...
    dt = 0.1 #increments of time value
    dt2 = 0.01
    ...
    dry_rocket_cg = 6.5*12 #from feet to inches, measured from top of nose cone
...
thrust_lbf = 700 #determines initial parameters, can be changed
rocket_radius_in = 3.25
fuel_mass_lb = 35.5 #defined from the RPL spec sheet
dry_rocket_mass_lb = 57.7
